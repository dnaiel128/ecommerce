...
router.afterEach(async (to,from) => {
    if((to.fullPath.includes('product/')) && (from.fullPath.includes('product/')) && to.fullPath!=from.fullPath) {
        /*const match = to.fullPath.match(/\/product\/(\d+)/);
        router.push({name:'ProductView', params: {id: match[1]}})   
        router.go()*/
        emit('reload-event')
    }
})
